document.getElementById('encryptButton').addEventListener('click', function() {
    ...
    if (!key || !text) {
        document.getElementById('message').innerText = 'Vui lòng nhập khóa và nội dung!';
        return;
    }
    const encryptedText = vigenereEncrypt(text, key);
    document.getElementById('encryptedText').value = encryptedText;
    document.getElementById('message').innerText = 'Mã hóa thành công!';
});
...
document.getElementById('saveButton').addEventListener('click', function() {
    const blob = new Blob([document.getElementById('encryptedText').value], { type: 'text/plain' });
